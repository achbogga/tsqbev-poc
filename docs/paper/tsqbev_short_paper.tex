...
The repository now includes a bounded local research loop on the official \texttt{nuScenes v1.0-mini} split~\cite{nuscenes}. After an initial proof-of-pipeline sweep, the loop was strengthened in two ways: the routed query bank was prevented from collapsing to near-ego LiDAR seeds, and recipe selection switched from surrogate loss to official \texttt{mini\_val} metrics. The current loop evaluates three six-epoch variants: a balanced frozen MobileNetV3 baseline, a proposal-heavy frozen MobileNetV3 variant, and a proposal-heavy frozen EfficientNet-B0 variant.
