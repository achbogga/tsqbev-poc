...
\midrule
Balanced frozen MobileNetV3 & 6 & 20.9826 & 0.0 & 0.0 \\
Proposal-heavy frozen MobileNetV3 & 6 & 22.4723 & 0.0 & 0.0 \\
Proposal-heavy frozen EfficientNet-B0 & 6 & 23.6836 & 0.0 & 0.0127 \\
\bottomrule
\end{tabular}
\end{table}

The strengthened sweep produced the first clearly nonzero official \texttt{NDS} in the repo while keeping a stable multimodal routed query mix of roughly one-third LiDAR, one-half proposal, and one-sixth global queries. That is still not a competitive detector: official \texttt{mAP} remains zero. However, it is meaningful evidence that the direction is now learnable under official metrics rather than only under surrogate validation loss. In particular, the lowest-loss recipe still had \texttt{NDS}=0.0, which justifies ranking recipes by official metrics instead of loss alone.
...
\item The current best official \texttt{mini\_val} result still has \texttt{mAP}=0.0, so the repo now explicitly blocks 10x compute scale-up until stronger gates are cleared.
...
\section{Conclusion}
The current \texttt{tsqbev-poc} repository now satisfies a stronger milestone than the original pre-research target: it has a minimal multimodal sparse-query architecture, a tested export path, a successful TensorRT engine build, a bounded local research loop on \texttt{nuScenes v1.0-mini}, and the first nonzero official \texttt{NDS} under a stabilized multimodal routing regime. The next engineering step is not blind scale. It is to bootstrap the student with a public pretrained external LiDAR teacher, with CenterPoint/OpenPCDet-style models as the first target~\cite{centerpoint,opencpdet}, and require a material official-metric lift before larger-scale training is authorized.
