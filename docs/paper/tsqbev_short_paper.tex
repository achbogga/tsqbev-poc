...
The repository now includes a bounded local research loop on the official \texttt{nuScenes v1.0-mini} split~\cite{nuscenes}. The loop evaluated three small variants for one epoch each: a frozen pretrained MobileNetV3 baseline, a higher-batch frozen variant, and an unfrozen-backbone variant. The frozen baseline with batch size 2 and gradient accumulation 2 achieved the best validation loss and was promoted to a longer run.

\begin{table}[t]
\centering
\caption{Bounded \texttt{nuScenes v1.0-mini} research sweep and promoted baseline.}
\label{tab:mini}
\begin{tabular}{lrrrr}
\toprule
Run & Epochs & Val total & mAP & NDS \\
\midrule
Frozen MobileNetV3 sweep winner & 1 & 28.0525 & 0.0 & 0.0 \\
Promoted mini baseline & 4 & 24.4006 & 1.54e-05 & 7.69e-06 \\
\bottomrule
\end{tabular}
\end{table}

The promoted mini baseline is functional but still undertrained. Its official \texttt{mini\_val} evaluation produced a nonzero car AP at the 4\,m threshold while the remaining classes stayed at zero. That is not a competitive result, but it is meaningful evidence that the end-to-end public training, checkpointing, prediction export, and official local evaluation path all complete successfully.
...
\section{Conclusion}
The current \texttt{tsqbev-poc} repository now satisfies a stronger milestone than the original pre-research target: it has a minimal multimodal sparse-query architecture, a tested export path, a successful TensorRT engine build, a bounded local research loop on \texttt{nuScenes v1.0-mini}, and a promoted mini baseline with nonzero official detection metrics. The next engineering step is to improve detector quality while preserving the same evidence-first workflow, then repeat the validation on larger public splits and the intended embedded target.
