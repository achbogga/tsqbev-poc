...
\begin{abstract}
This paper proposes a frontier-reset program for a deployable multimodal 3D perception student.
The current local control in \texttt{tsqbev-poc} reaches \texttt{NDS 0.1833} and \texttt{mAP 0.1814}
on \texttt{nuScenes mini\_val}; a first \texttt{DINOv3} probe reaches \texttt{NDS 0.1616} and
\texttt{mAP 0.1591}; and a reproduced public \texttt{BEVFusion} system provides a much stronger,
cross-split contextual ceiling on the full validation set. This evidence suggests that the main
bottleneck is no longer basic non-collapse, but the absence of a world-aligned bridge between
frontier image priors and precise 3D geometry. We therefore propose a research program centered on a
\emph{world-anchored latent contract}: an efficient \texttt{Sparse4D}-style student, a
\texttt{DINOv3}-conditioned camera branch with \texttt{BEVFormer v2}-style perspective supervision,
dense world-aligned distillation from unrestricted teachers such as \texttt{BEVFusion} and
\texttt{OpenPCDet}, and offline region-support priors from \texttt{SAM 2.1}. The proposal is built
around three hypotheses: camera foundation features need an explicit geometry bridge, lane learning
should only be reintroduced after the shared world latent is stable, and the research control plane
must be governed by official metrics rather than by loss alone. We define the model, the
distillation losses, the staged multitask curriculum, the deployment path, and falsifiable success
criteria for the next frontier push.
\end{abstract}
...
\texttt{NDS 0.7072}, \texttt{mAP 0.6730} on the full validation split, so it should be treated as a
contextual ceiling rather than a split-matched comparison. The first real \texttt{DINOv3} probe,
...
reaching zero on \texttt{mini\_val}; this is strong evidence against resuming naive multitask
coupling without stricter task isolation and checkpointing.

This proposal makes three core contributions.
\begin{enumerate}[leftmargin=*,itemsep=2pt]
\item It proposes a \emph{world-aligned} frontier hypothesis rather than a backbone-only one. This means a
shared latent in world or BEV coordinates that every teacher supervises explicitly.
\item It separates cross-modal fusion from temporal memory. Gated or deformable
...
spatial alignment.
\item It treats the research lab itself as a first-class system. Exact memory, semantic retrieval,
...
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.31\linewidth} >{\raggedright\arraybackslash}p{0.13\linewidth} r r >{\raggedright\arraybackslash}X}
\toprule
...
blocked by complete model failure. There is a real local control, a real \texttt{DINOv3} signal,
and a real dense-teacher ceiling. Second, the local frontier is not yet close enough to justify
incrementalism. The full-validation \texttt{BEVFusion} result is not directly comparable to the
\texttt{mini\_val} student numbers, but it is large enough to make clear that small local
schedule-only nudges are unlikely to close the broader gap.
...
We expand the main abbreviations on first use. \textbf{NDS} is the nuScenes Detection Score;
\textbf{mAP} is mean Average Precision; \textbf{SSM} denotes a state-space model; \textbf{MoE}
denotes a mixture-of-experts model; and \textbf{KB} refers to the repository knowledge base. Later
loss terms expand \textbf{VFL} as Varifocal Loss, \textbf{BCE} as binary cross-entropy, and
\textbf{GIoU} as generalized intersection-over-union.
...
SSMs are \emph{internal temporal memory} mechanisms \cite{flamingo,mamba}. This distinction matters
for design: we therefore use gated or deformable fusion for spatial cross-modal alignment now,
...
\node[student] (world) at (8.9,1.3) {shared world latent\\$Z_s^{\text{world}}$};
...
replacing the deployable student. Runtime inference is the green path; the blue blocks are
training-time teachers. The control plane is shown separately in Figure~\ref{fig:control}.}
\label{fig:proposal}
\end{figure*}
...
gated deformable cross-attention block fuses camera support into the world latent
\cite{deformabledetr}. The dense teacher side supplies three different kinds of supervision:
\begin{enumerate}[leftmargin=*,itemsep=2pt]
...
\end{equation}
Equation~\eqref{eq:gated-cross} encodes the key distinction from the knowledge base: cross-attention is the
...
\label{eq:quality}
\end{equation}
where \(q_t\) is a quality-aware teacher target, \(s_s\) is the student ranking score,
\(\mathrm{VFL}\) denotes Varifocal Loss, and \(\mathrm{GIoU}\) denotes generalized
intersection-over-union \cite{varifocalnet,giou}. The tuple \((c,d,b)\) denotes center, dimensions,
and box geometry.
...
\label{eq:occ}
\end{equation}
Here \(\mathrm{BCE}\) is binary cross-entropy. This loss exists to preserve object support and foreground structure that box-only supervision
...
\label{tab:stages}
\small
\begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}p{0.07\linewidth} >{\raggedright\arraybackslash}p{0.16\linewidth} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
\toprule
...
S1 & stronger camera-to-world bridge & \texttt{DINOv3} + projection adapter + perspective supervision & if official \texttt{mini\_val} stays below the trusted control after teacher distillation, stop backbone-only work \\
S2 & stronger world supervision & \texttt{BEVFusion/OpenPCDet} BEV distillation + quality targets + \texttt{SAM 2.1} region priors & if geometry and export sanity do not improve, stop adding teacher losses blindly \\
S3 & stable lane integration & staged \texttt{MapTRv2} vector head with non-regression gate & if detection drops beyond tolerance, revert to the frozen shared trunk \\
...
The proposal's central hypothesis is this:
\begin{quote}
\textbf{The smallest deployable student is most likely to break the local frontier if all heavy teachers
...
The knowledge base explicitly distinguishes fusion from temporal memory. Mamba-style SSMs may eventually
replace or compress temporal attention, but they do not solve the current image-to-world bridge
problem \cite{mamba}. Starting there would likely attack the wrong bottleneck first.
...
\emph{VarifocalNet: An IoU-aware Dense Object Detector}. CVPR 2021.
\url{https://arxiv.org/abs/2008.13367}

\bibitem{giou}
\emph{Generalized Intersection over Union: A Metric and A Loss for Bounding Box Regression}. CVPR 2019.
